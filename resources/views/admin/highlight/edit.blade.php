...
{{-- {{ $type = '' }} --}}
@if(Storage::exists($highlight->url))
    {{ $type = 'file' }}
@else
    {{ $type = 'url' }}
@endif
            ...
            <div class="card-body">
                <form action="/admin/highlight/{{ $highlight->id }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row mb-4">
                        ...
                        <div class="col-sm-10">
                            <input type="text" class="form-control @error('title') is-invalid @enderror" id="title" name="title" required value="{{ old('title',$highlight->title) }}" autocomplete="off">
                            @error('title')
                    ...
                    <div class="row mb-4" id="input">
                        ...
                        <div class="col-sm-10">
                            <input type="{{ $type == 'url' ? 'text' : 'file' }}" class="form-control @error('url') is-invalid @enderror" id="url" name="url" required value="{{ old('url',$highlight->url) }}" autocomplete="off">
                            @error('url')
                            <div class="invalid-feedback">
                                {{ $message }}
                            </div>
                    ...
                    <div class="row justify-content-end">
                        <div class="col-sm-10">
                            ...
                            <a href="/admin/highlight" class="btn btn-danger px-5 me-2 float-end btn-sm">Cancel</a>
                        </div>
                    </div>
                </form>
            </div>
